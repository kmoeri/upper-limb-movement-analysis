...
#
#     # select the options for the landmark object
#     base_options = python.BaseOptions(model_asset_path=model_path, delegate=python.BaseOptions.Delegate.CPU) # better for Apple Silicon (M1/M2/M3)
#     #base_options = python.BaseOptions(model_asset_path=model_path, delegate=python.BaseOptions.Delegate.GPU) # for gpu support
#     options = vision.PoseLandmarkerOptions(
#         base_options=base_options,
#         running_mode=mp.tasks.vision.RunningMode.VIDEO,
#         min_pose_detection_confidence=min_pose_detect_conf,
...
#         print("Extracting hand landmarks...")
#
#         # sudden hand swaps (R -> L and/or L -> R) can be a problem, which is why they are compared with the previous frame
...
#
#             # perform hand landmark detection on current frame
#             # passing the timestamp for smoothing in "VIDEO" mode
#             detection_result = detector.detect_for_video(mp_image, frame_timestamp_ms)
#
#             # initialize marker list for 42 landmarks (21 Left, 21 Right)
#             marker_lst: list = [(None, None, None)] * 42
...
#             handedness_lst = detection_result.handedness
#
#             # track the assigned hand in the current frame
#             assigned_this_frame = {'Left': False, 'Right': False}
#             tmp_prev_wrists = prev_wrists.copy()
#
#             # iterate over both sides (two lists with 21 landmarks each)
#             for idx in range(len(hand_landmark_lst)):
#                 hand_landmarks = hand_landmark_lst[idx]
#                 handedness = handedness_lst[idx][0].category_name   # 'Left' or 'Right'
#
#                 # wrist landmark is used as a reference point
#                 wrist_pos = np.array([hand_landmarks[0].x, hand_landmarks[0].y])
#
#                 final_label = handedness
#
#                 # case 1) swap by spatial jump check
#                 if prev_wrists['Left'] is not None and prev_wrists['Right'] is not None:
#                     dist_to_L = np.linalg.norm(wrist_pos - prev_wrists['Left'])
#                     dist_to_R = np.linalg.norm(wrist_pos - prev_wrists['Right'])
#
#                     # override prediction if it is a clear swap to the other hand
#                     if handedness == 'Right' and dist_to_L < dist_to_R:
#                         final_label = 'Left'
#                     elif handedness == 'Left' and dist_to_R < dist_to_L:
#                         final_label = 'Right'
#
#                 # case 2) both hands detected and assigned on the same hand
#                 if assigned_this_frame[final_label]:
#                     final_label = 'Right' if final_label == 'Left' else 'Left'
#
#                 # mark label as used and update previous wrist position
#                 assigned_this_frame[final_label] = True
#                 tmp_prev_wrists[final_label] = wrist_pos
#
#                 # determine the base index based on the corrected label; 0-20 for left, 21-41 for right
#                 base_index = 0 if final_label == 'Left' else 21
#
#                 # iterate over each landmark of the current hand
#                 for i, landmark in enumerate(hand_landmarks):
#                     marker_lst[base_index + i] = (landmark.x, landmark.y, landmark.z)
#
#             # update previous wrists for the next frame
#             prev_wrists['Left'] = tmp_prev_wrists['Left'] if assigned_this_frame['Left'] else None
#             prev_wrists['Right'] = tmp_prev_wrists['Right'] if assigned_this_frame['Right'] else None
#
#             # store the landmark data of each frame in a dict with the frame number as key)
#             base_key: str = str('0' * f_digits)
#             key: str = base_key[:-len(str(fnum))] + str(fnum)
...
# #                              3) HOLISTIC TRACKER                              #
# # ============================================================================= #
# # --- 3) NORMALIZED LANDMARKS (Legacy Tracker) -------------------------------- #
